...
const DOT_COLORS: Record<keyof typeof VARIANTS, string> = {
  NEW: "bg-blue-500", QUALIFIED: "bg-amber-500", PROPOSAL: "bg-orange-500",
  WON: "bg-emerald-500", LOST: "bg-red-500",
  success: "bg-emerald-500", warning: "bg-amber-500", danger: "bg-red-500",
  info: "bg-blue-500", default: "bg-neutral-400",
};

const SIZES = { sm: "text-xs px-2 py-0.5 gap-1", md: "text-sm px-2.5 py-1 gap-1.5" } as const;
...
export default function Badge({
  variant = "default", size = "sm", dot = false, label, children, className = "",
}: BadgeProps) {
  ...
  return (
    <span className={["inline-flex items-center font-medium rounded-full ring-1", meta.cls, SIZES[size] ?? SIZES.sm, className].join(" ")}>
