...
function AvatarSkeleton({ size = "md" }) {
  const s = { sm: "w-8 h-8", md: "w-10 h-10", lg: "w-12 h-12" }[size] ?? "w-10 h-10";
  ...
  return (
    <div className="space-y-8 animate-pulse">
      ...
      <div className="bg-white rounded-xl border border-neutral-200 p-6 space-y-4">
        <Base className="h-5 w-40" />
        <Base className="h-64 w-full rounded-lg" />
      </div>

      {/* Table */}
      <div className="bg-white rounded-xl border border-neutral-200">
        <div className="px-6 py-4 border-b border-neutral-100">
          <Base className="h-5 w-48" />
        </div>
        <table className="w-full">
          <tbody>
            {[...Array(5)].map((_, i) => <TableRow key={i} cols={4} />)}
          </tbody>
        </table>
      </div>
    </div>
