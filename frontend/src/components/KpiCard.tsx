...
interface Trend { direction: "up" | "down"; value: number; label?: string; }
...
export default function KpiCard({ title, value, icon: Icon, color = "indigo", trend }: KpiCardProps) {
  ...
  return (
    <div className="group bg-white rounded-2xl border border-neutral-100 shadow-sm px-5 py-5 flex items-center gap-4 transition-all duration-200 hover:shadow-md hover:border-neutral-200 cursor-default overflow-hidden">
      <div className={["flex items-center justify-center w-12 h-12 rounded-xl shrink-0 ring-1 transition-transform duration-200 group-hover:scale-110", c.bg, c.ring].join(" ")}>
      ...
      <div className="min-w-0 flex-1">
        ...
        {trend ? (
          <p className={["flex items-center gap-0.5 mt-1.5 text-xs font-semibold", trendUp ? "text-emerald-600" : "text-red-500"].join(" ")}>
            <TrendIcon size={12} strokeWidth={2.5} />
            <span>{Math.abs(trend.value)}%</span>
            <span className="font-normal text-neutral-400 ml-0.5">{trend.label ?? "geçen aya göre"}</span>
          </p>
