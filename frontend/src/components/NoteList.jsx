...
function Avatar({ name }) {
  ...
  return (
    <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold shrink-0 ${color}`}>
      {letter}
    </div>
  );
}

export default function NoteList({ notes, customerId, onRefresh }) {
              ...
              <div className="flex-1 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
                <div className="flex items-center justify-between gap-2 mb-1.5 flex-wrap">
                  <span className="text-xs font-semibold text-gray-700">
                    {note.created_by_name || "Kullanıcı"}
                  </span>
