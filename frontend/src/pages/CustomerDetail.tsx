...
interface InfoFieldProps {
  ...
  type?: string;
  editValues: CustomerFormData;
  onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
}

function InfoField({ label, value, editMode, name, type = "text", editValues, onChange }: InfoFieldProps) {
  if (!editMode) {
    return (
      <div>
...
export default function CustomerDetail() {
  ...
  const [editValues, setEditValues]   = useState<CustomerFormData>({ first_name: "", last_name: "", email: "", phone: "", company: "" });
  ...
  const fetchCustomer = useCallback(async () => {
    ...
    try {
      ...
      if (typeof err === "object" && err !== null && "response" in err && (err as { response?: { status?: number } }).response?.status === 404) setNotFound(true);
  ...
  const handleSave = async () => {
    setEditLoading(true);
    try {
      const { data } = await api.patch<Customer>(`/customers/${id}/`, editValues);
      setCustomer((prev) => prev ? ({ ...prev, ...data }) : data);
      setEditMode(false);
      toast.success("Müşteri güncellendi.");
    } catch (err: unknown) {
      const axErr = err as { response?: { data?: Record<string, string | string[]> } };
      const first = axErr.response?.data && (Object.values(axErr.response.data).flat()[0] as string | undefined);
