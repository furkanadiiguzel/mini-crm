...
import type { DashboardData, DashboardStageRow } from "../types/api";
import type { CardColor } from "../components/KpiCard";
...
function BarTooltip({ active, payload }: { active?: boolean; payload?: { payload: DashboardStageRow & { label: string; color: string } }[] }) {
...
function DonutTooltip({ active, payload }: { active?: boolean; payload?: { payload: DashboardStageRow & { label: string; color: string } }[] }) {
  if (!active || !payload?.length) return null;
  const d = payload[0].payload;
  return (
...
type ChartRow = DashboardStageRow & { label: string; color: string };
...
function ChartSection({ chartData }: { chartData: ChartRow[] }) {
...
function WelcomeBanner({ user, activeOpportunities }: { user: import("../types/auth").User | null; activeOpportunities: number }) {
...
function KpiSection({ data, activeOpportunities }: { data: DashboardData; activeOpportunities: number }) {
  const cards = [
    {
      ...
      trend: data.new_customers_last_30_days > 0
        ? { direction: "up" as const, value: data.new_customers_last_30_days, label: "yeni son 30 günde" }
        : undefined,
    },
    {
...
export default function Dashboard() {
  ...
  const chartData = (data?.opportunities_by_stage ?? []).map((s) => ({
    ...s,
    label: STAGE_META[s.stage as keyof typeof STAGE_META]?.label ?? s.stage,
    color: STAGE_META[s.stage as keyof typeof STAGE_META]?.color ?? "#6B7280",
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      {/* KPI Cards */}
      {data && <KpiSection data={data} activeOpportunities={activeOpportunities} />}
